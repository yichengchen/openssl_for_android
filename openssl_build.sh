...
then
    ...
    ./Configure android-arm -D__ANDROID_API__=${ANDROID_TARGET_API} -static no-asm no-shared no-tests  no-sock no-deprecated no-afalgeng no-async no-autoalginit no-autoerrinit no-capieng no-cms no-dgram no-dynamic-engine no-engine no-ec2m no-filenames no-gost no-hw-padlock no-nextprotoneg no-ocsp no-psk no-rfc3779 no-srp no-srtp no-ts --prefix=${OUTPUT_PATH}
    build_library
